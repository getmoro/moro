...
const syncTable = (type) => {
  return new Promise((resolve, reject) => {
    axios.post(constants.SYNC_SERVER)
      .then((response) => {
        console.log(response.data);
        // check the 'log' table exists
        db.getLastLogData(db.knex, type)
          .then((lastLog) => {
            ...
            db.getRowsAfter(db.knex, type, lastLogId)
              .then((recentLogs) => {
                // send them to the server
                if (recentLogs[recentLogs.length - 1]) {
                  console.log(constants.TEXT.sync.syncing + type + '...');
                  axios.post(constants.SYNC_SERVER, { type, records: recentLogs })
                    .then((response) => {
                      console.log(response.data);
                      // save a new log in 'logs' table if successful
                      db.setLogData(db.knex, { type, lastLogId: recentLogs[recentLogs.length - 1].id })
                        .then((result) => {
