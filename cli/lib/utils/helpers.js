...
const shouldWorkUntil = (start, CONFIG, breakDuration) => {
  ...
  if (!spinner.enabled) {
    console.log(
      `Working until ${goHomeTime} will make it a full (${CONFIG.HOURS_IN_A_WORK_DAY} hours) day\n\n`,
    );
  }
  ...
  spinner
    .stopAndPersist({
      symbol: '⏰',
      text: `Working until ${goHomeTime} will make it a full (${CONFIG.HOURS_IN_A_WORK_DAY} hours) day\n\n`,
    })
    .start();
};
...
const sumRow = (sum, week) => [{ content: `Sum in week ${week}` }, formatSum(sum)];
