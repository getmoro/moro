...
const formatWorkHours = (workHours) => {
  ...
  return toString(workHours.hours(), workHours.minutes())
}
...
const buildTable = (records, pattern) => {
  ...
  let sum
  let week = -1
  const table = new Table({head: ['Week', 'Date', 'Work Hours'], style: {head: [], border: []}, colWidths: [10, 15, 28]})
  records.forEach((record) => {
    ...
    table.push([wn, date.format(pattern), formatWorkHours(workHours)])
...
const sumRow = (sum, week) => {
  return [{colSpan: 2, content: `Sum in week ${week}`}, formatSum(sum)]
}
